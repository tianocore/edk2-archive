...
/**
  ...
  @param  value  Indicates that the platform implements no support for Hardware
                 Error Record Persistence.
  @param  value  Indicates that the platform implements Hardware Error Record
                 Persistence.
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "HwErrRecSupport: Can not set the variable\n"));
  }
}
